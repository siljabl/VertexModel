...
def plot_frame(vm, fig=None, ax=None, update=True, only_set=False,
               vertex_indices=False, cbar_zero='hexagon', hmax=14):
    """
    Plot vertex model.
    ...
    # SET COLORBAR
    cmap = cm.batlowW
    norm_area = Normalize(0, hmax)                                    # interval of value represented by colourmap
    scalarMap = ScalarMappable(norm_area, cmap)                       # conversion from scalar value to colour
    ...
    if ("t0" in locals() or "sT0" in locals()):
        ...
        if tensions_std != 0:
            lines.set_color(list(map(
                lambda s_tension: scalarMap.to_rgba(s_tension),
